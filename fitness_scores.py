	    ...
	    counter=0
            with open(filename,'r') as barcode_reads: 
                for line in barcode_reads: # for each barcode
                    ...
                    try: 
                        barcode_counts[line[0:26]][filenumber]+=1 # increment the count of that barcode in the count dictionary
...
def calc_fitness(filelist_filename, count_threshold): # read in the minimum number of initial counts necssary to include a given barcode in the analysis
	...
	for line in filelist: # for each replicate
	    ...
	    with open(barcode_counts_filename,'r') as barcode_counts_file, open(total_counts_filename,'r') as total_counts_file:
		    ...
		    for t in range(3,len(stuff)):
			count_dict[stuff[0]][t-1]+=np.sqrt(count_dict[stuff[0]][2]) # add a small-count correction

		WT_slopes={} # identify the WT-mapped barcode with the median fitness score
		for barcode in count_dict:
		    if count_dict[barcode][1] == 'WT':
			if count_dict[barcode][2] >= 200: # consider only well-represented barcodes
			    WT_slopes[stats.linregress(x_values,count_dict[barcode][2:])[0]]=barcode
		if len(WT_slopes.keys()) % 2 == 0: # make sure there is an odd number of candidates
		    del WT_slopes[WT_slopes.keys()[0]]
		median_slope=np.median(WT_slopes.keys())
		median_barcode=WT_slopes[median_slope]
	        ...
	        for barcode in count_dict: # for each barcode
	            if count_dict[barcode][2] > int(count_threshold): # consider only barcodes that start with at least a given number of counts
	                ...
	                for t in range(2,len(count_dict[barcode])): # for each timepoint
	                    ...
	                    freq_dict[barcode].append(float(count_dict[barcode][t]/count_dict[median_barcode][t])) # convert each count to frequency
...
# calculate p-values for t-tests compared to WT
	...
	fitness_scores_2={}
	for variant in fitness_scores:
	    if not math.isnan(fitness_scores[variant][3]):
		fitness_scores_2[variant]=fitness_scores[variant]
	...
	for variant in fitness_scores_2:
	    fitness_scores_2[variant][0]=fitness_scores_2[variant][0]
	fitness_scores[(0, 'WT')][0]=0


        with open(filelist_filename[:-4]+'_scores.pkl','w') as fout: # output the data as a pickle
            fout.write(pickle.dumps(fitness_scores_2))
        with open(filelist_filename[:-4]+'_scores.txt','w') as fout2: # and as text
            fout2.write('Position, Substitution, Fitness Score, Standard Deviation, Number of Mapped Barcodes, P-value (relative to WT)')
            for variant in fitness_scores_2:
                fout2.write('\n'+str(variant[0])+','+str(variant[1]))
                for item in fitness_scores_2[variant]:
                     fout2.write(','+str(item))


# Make a heatmap for the deep mutational scanning data
def make_heatmap(filename): # requires the number of residues from user and the WT median from calc_fitness

    fitness_scores=pickle.load(open(filename,'r')) # load the fitness score file
    ...
    amino_acids=20
    fitness_array=np.zeros(shape=(amino_acids,140)) # create an array the size of the protein
	...
	fitness_array[aa_to_number[variant[1]]][variant[0]-1]=fitness_scores[variant][0]
    ...
    np.savetxt(filename[:-3]+'csv',fitness_array,delimiter=',')
